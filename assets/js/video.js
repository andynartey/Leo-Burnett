...
var VideoJS = JRClass.extend({
    ...
    init: function (element, setOptions) {
      ...
      //  alert($(this.video).attr('id'));


        //alert($(this.video).parent().parent().parent().parent().hasClass('showReel'));

        var topviddiv = $(this.video).parent().parent().parent().parent();

        var emailBtn = $(topviddiv).find('.emailBtn');
       // alert($(emailBtn).find('a').attr('href'));

        var twitterBtn = $(topviddiv).find('.twitterBtn');
       // alert(twitterBtn.find('.PE_newWindow').attr('href'));

        var faceBookBtn = $(topviddiv).find('.faceBookBtn');
      //  alert(faceBookBtn.find('.PE_newWindow').attr('href'));

        this.options.caseId = $(this.video).attr('id');
        this.options.mailAddress = $(emailBtn).find('a').attr('href');
        this.options.twitterShareUrl = twitterBtn.find('.PE_newWindow').attr('href');
        this.options.facebookShareUrl = faceBookBtn.find('.PE_newWindow').attr('href');
    ...
    html5Init: function () {
        ...
        if (VideoJS.isIOS()) {
            ...
            this.options.useBrowserControls = false;
    ...
    buildM7ControlBar: function () {
        ...
        */
        ...
        // Create a div element to hold the different M7 controls
        this.extendedControls = _V_.createElement("div", { className: "M7_extendedControls" });
        // Build the embed control
        this.titleControl = _V_.createElement("h4", {
            innerHTML: "Share this..."
        });
        this.extendedControls.appendChild(this.titleControl)
        // Create a list element to hold the different M7 controls
        this.socialControls = _V_.createElement("ul", { className: "shareButtons" });
        // Add the controls to the extended controls's container
        this.extendedControls.appendChild(this.socialControls);
        // Add the extended controls to the video's container
        this.video.parentNode.appendChild(this.extendedControls);

        // Build the close control
        this.closeControl = _V_.createElement("li", {
            className: "closeBtn last",
            innerHTML: "<span>Close</span>"
        });
        this.socialControls.appendChild(this.closeControl);

        // Build the email control
        this.emailControl = _V_.createElement("li", {
            className: "emailBtn",
            innerHTML: "<a href='" + this.options.mailAddress + "'>email page</a>"
           /*innerHTML: "<a href='mailto:" + this.options.mailAddress + "&subject=" + this.options.mailSubject + "&body=" + this.options.mailBody + "'>email page</a>"*/
        });
        this.socialControls.appendChild(this.emailControl);

        // Build the facebook control
        this.fbControl = _V_.createElement("li", {
            className: "faceBookBtn",
             innerHTML: "<a class='PE_newWindow' href='" + this.options.facebookShareUrl + "'>Facebook</a>"
           /* innerHTML: "<a class='PE_newWindow' href='http://www.facebook.com/sharer.php?u=" + this.options.facebookShareUrl + "'>Facebook</a>"*/
        });
        this.socialControls.appendChild(this.fbControl);

        // Build the twitter control
        this.twittControl = _V_.createElement("li", {
            className: "twitterBtn",
             innerHTML: "<a class='PE_newWindow' href='" + this.options.twitterShareUrl + "'>Twitter</a>"
            /*innerHTML: "<a class='PE_newWindow' href='http://twitter.com/?status=" + this.options.twitterStatus + " " + this.options.twitterShareUrl + "'>Twitter</a>"*/
        });
        this.socialControls.appendChild(this.twittControl);

        // Build the embed control
        this.embedControl = _V_.createElement("li", {
            className: "embedBtn last",
            innerHTML: "<a class='PE_embed' href='#" + this.options.caseId + "-caseStudy-embed'>Embed</a>"
        });
        this.socialControls.appendChild(this.embedControl);

        this.initializeM7Controls();
    },
    // Set up M7 Custom Event Listeners
    initializeM7Controls: function () {
        // emailControl
        this.emailControl.addEventListener("click", this.onEmailClick.context(this), false);
        // embedControl
        this.embedControl.addEventListener("click", this.onEmbedClick.context(this), false);
        // twittControl
        //this.twittControl.addEventListener("click", this.onTwittClick.context(this), false);
        // fbControl
        //this.fbControl.addEventListener("click", this.onfbClick.context(this), false);
        // closeControl
        this.closeControl.addEventListener("click", this.onCloseClick.context(this), false);
    },
    showControlBar: function () {
        if (!this.options.showControlsAtStart && !this.hasPlayed) { return; }
        this.controls.style.display = "block";
        this.extendedControls.style.display = "block";
        this.positionControlBar();
    },
    ...
    positionControlBar: function () {
        ...
        if (this.options.controlsBelow) {
            ...
            this.extendedControls.style.top = (this.video.offsetHeight - 500) + "px";
        } else {
            this.controls.style.top = (this.video.offsetHeight - this.controls.offsetHeight) + "px";
            this.extendedControls.style.top = ((this.video.offsetHeight - this.controls.offsetHeight) - 494) + "px";
        }
    ...
    hideControlBar: function () {
        if (this.options.controlsHiding && !this.mouseIsOverControls) {
            ...
            this.extendedControls.style.display = "none";
        }
    },
    ...
    onExtendedControlsMouseMove: function () {
        // Block controls from hiding when mouse is over them.
        this.mouseIsOverControls = true;
    },
    onExtendedControlsMouseOut: function (event) {
        this.mouseIsOverControls = false;
        // Have to add the video mouseout to the controller too or it may not hide.
        this.onVideoMouseOut(event);
    },
    ...
    onEmbedClick: function (event) {
        this.video.pause();
        M7.PE.videoEmbed(this.video, 'embed');
    },
    onEmailClick: function (event) {
        this.video.pause();
        //M7.PE.videoEmail(this.video, 'embed');
    },
    onCloseClick: function (event) {
        this.video.currentTime = 0;
        this.video.pause();
        M7.PE.videoClose(this.video, false);
    },
    ...
    initializeBigPlayButton: function () {
        ...
        this.video.addEventListener("loadstart", this.bigPlayButtonOnLoad.context(this), false);
    ...
    bigPlayButtonOnLoad: function (event) { this.hideBigPlayButton(); },
    ...
    play: function () {
        ...
        this.loadTimeout = setTimeout(function () { this.hideControlBar(); } .context(this), 4000);
    },
